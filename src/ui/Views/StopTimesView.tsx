...
export interface IStopTimesViewPropsWithIStops
  extends IStopTimesViewCommonProps {
  readonly stops: readonly LocalIStop[];
}
...
const duplicatePruneMethods: {
  [pruneType: string]: (
    stopsTimes: readonly IStopTime[],
    stops: readonly StopId[],
  ) => IStopTime[];
} = {
  byFirstDeparture: (stopsTimes: readonly IStopTime[]) =>
  ...
  byShortestTravelTime: (stopsTimes: readonly IStopTime[]) => {
  ...
  byStopOrder: (stopsTimes: readonly IStopTime[], stops: readonly StopId[]) =>
    stopsTimes.reduce((acc: IStopTime[], curr: IStopTime) => {
      const foundDuplicate = acc.find(
        stopTime =>
          stopTime.trip &&
          curr.trip &&
...
const StopTimesView: React.SFC<ICombinedStopTimesViewProps> = (
  ...
  let { monitorConfig } = props as IStopTimesViewCommonProps;
  if (!monitorConfig) {
            ...
            const finalStopTimes = duplicatePrunedStopTimes
              ...
              .map(stopTime => {
                if (
                  ...
                  const foundIStop:
                    | LocalIStop
                    | undefined = (props as IStopTimesViewPropsWithIStops).stops.find(
                    stop => stop.gtfsId === stopTime.stop!.gtfsId,
                  );
                  return {
                    ...stopTime,
                    stop: {
